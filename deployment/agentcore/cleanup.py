...
Examples:
    python cleanup.py bedrock-sonic
...
class AgentCoreCleanup:
    ...
    def delete_observability(self):
        """Delete AgentCore Observability destination if it was created during deployment."""
        if not self.config or 'observability' not in self.config:
            self._info("\nNo observability configuration found, skipping observability cleanup")
            return

        dest_id = self.config['observability'].get('destination_id')
        dest_name = self.config['observability'].get('destination_name', 'unknown')
        region = self.config.get('aws_region', 'us-east-1')

        if not dest_id:
            self._info("No observability destination ID found, skipping cleanup")
            return

        self._print(f"\n📊 Deleting AgentCore Observability destination: {dest_name} ({dest_id})", Colors.YELLOW)
        ...
        try:
            ...
            self._success(f"Observability destination {dest_id} deleted")
        except Exception as e:
            if "not found" in str(e).lower() or "ResourceNotFoundException" in str(e):
                self._info("Observability destination already deleted or not found")
            else:
                self._info(f"Failed to delete observability destination: {e}")
    ...
    def delete_memory(self):
        ...
        self._print(f"\n🧠 Deleting AgentCore Memory: {memory_name} ({memory_id})", Colors.YELLOW)
    ...
    def delete_subagents(self):
        """Delete A2A sub-agent runtimes deployed for the strands-sonic workshop"""
        if self.websocket_folder != 'strands-sonic':
            return

        # Known sub-agent names
        subagent_names = [
            "voice_workshop_auth_agent",
            "voice_workshop_banking_agent",
            "voice_workshop_mortgage_agent",
        ]
        subagent_folders = ["auth_agent", "banking_agent", "mortgage_agent"]

        region = 'us-east-1'
        if self.config:
            region = self.config.get('aws_region', 'us-east-1')

        self._print("\n🤖 Deleting A2A Sub-Agents...", Colors.YELLOW)
        ...
        try:
            ...
            # Try to find agent IDs from multiple sources
            for sa_name, sa_folder in zip(subagent_names, subagent_folders):
                agent_id = None
                ...
                if sa_yaml.exists():
                    try:
                        with open(sa_yaml, 'r') as f:
                            sa_config = yaml_lib.safe_load(f)
                        if sa_config and 'agents' in sa_config:
                            for name, agent_cfg in sa_config['agents'].items():
                                bc = agent_cfg.get('bedrock_agentcore', {})
                                if bc and bc.get('agent_id'):
                                    agent_id = bc['agent_id']
                                    break
                    except Exception:
                        pass

                # Source 2: Check setup_config.json subagents list
                if not agent_id and self.config and 'subagents' in self.config:
                    for sa in self.config['subagents']:
                        if sa.get('name') == sa_name and sa.get('agent_arn'):
                            agent_id = sa['agent_arn'].split('/')[-1]
                            break

                # Source 3: Try listing runtimes
                if not agent_id:
                    try:
                        response = bedrock_client.list_agent_runtimes()
                        for rt in response.get('agentRuntimeSummaries', []):
                            if rt.get('agentRuntimeName') == sa_name:
                                agent_id = rt.get('agentRuntimeId')
                                break
                    except Exception:
                        pass

                if not agent_id:
                    self._info(f"Sub-agent {sa_name} not found, skipping runtime deletion")
                else:
                    self._info(f"Deleting sub-agent runtime: {sa_name} ({agent_id})")
                    try:
                        bedrock_client.delete_agent_runtime(agentRuntimeId=agent_id)
                        self._success(f"Sub-agent {sa_name} deleted")
                    except Exception as e:
                        if "ResourceNotFoundException" in str(e) or "not found" in str(e).lower():
                            self._info(f"Sub-agent {sa_name} already deleted")
                        else:
                            self._error(f"Failed to delete sub-agent {sa_name}: {e}")

            # Clean up sub-agent ECR repositories
            ecr_client = boto3.client('ecr', region_name=region)
            for sa_name in subagent_names:
                repo_name = f"bedrock-agentcore-{sa_name}"
                try:
                    ecr_client.delete_repository(repositoryName=repo_name, force=True)
                    self._success(f"ECR repo {repo_name} deleted")
                except Exception as e:
                    if "RepositoryNotFoundException" in str(e):
                        pass  # Already gone
                    else:
                        self._info(f"ECR repo {repo_name} cleanup skipped: {e}")

            # Clean up sub-agent CodeBuild projects
            cb_client = boto3.client('codebuild', region_name=region)
            for sa_name in subagent_names:
                project_name = f"bedrock-agentcore-{sa_name}-builder"
                try:
                    cb_client.delete_project(name=project_name)
                    self._success(f"CodeBuild project {project_name} deleted")
                except Exception as e:
                    if "ResourceNotFoundException" in str(e) or "not found" in str(e).lower():
                        pass
                    else:
            ...
            if subagents_dir.exists():
                for sa_folder in ["auth_agent", "banking_agent", "mortgage_agent"]:
                    ...
                    if config_file.exists():
                        config_file.unlink()
                        self._info(f"Removed {config_file}")

        except Exception as e:
            self._error(f"Sub-agent cleanup failed: {e}")
            import traceback
            traceback.print_exc()

    def delete_all_workshop_gateways(self):
        """Delete all gateways matching workshop prefixes (voice-agent-workshop-* or voice_workshop_*)"""
        self._print("\n🌐 Searching for all workshop gateways to delete...", Colors.YELLOW)
        ...
        try:
            ...
            prefixes = ('voice-agent-workshop-', 'voice_workshop_', 'voice-workshop-')
            workshop_gateways = [gw for gw in gateways if gw.get('name', '').startswith(prefixes)]
            
            if not workshop_gateways:
                self._info("No workshop gateways found")
                return
            
            self._info(f"Found {len(workshop_gateways)} workshop gateway(s) to delete:")
            for gw in workshop_gateways:
                self._info(f"   - {gw['name']} ({gw['gatewayId']})")
            
            for gw in workshop_gateways:
                gw_id = gw['gatewayId']
                gw_name = gw['name']
                self._info(f"\nDeleting gateway: {gw_name} ({gw_id})")
                
                # Delete targets first
                try:
                    targets_response = bedrock_client.list_gateway_targets(gatewayIdentifier=gw_id)
                    for target in targets_response.get('items', []):
                        target_id = target.get('targetId')
                        try:
                            bedrock_client.delete_gateway_target(gatewayIdentifier=gw_id, targetId=target_id)
                            self._success(f"  Target {target_id} deleted")
                        except Exception as e:
                            self._info(f"  Failed to delete target {target_id}: {e}")
                    ...
                    if targets_response.get('items'):
                        import time
                        self._info("  Waiting for targets to be fully deleted...")
                        time.sleep(10)
                except Exception as e:
                    self._info(f"  Failed to list targets: {e}")
                
                # Delete gateway
                try:
                    bedrock_client.delete_gateway(gatewayIdentifier=gw_id)
                    self._success(f"  Gateway {gw_name} deleted")
                except Exception as e:
                    if "ResourceNotFoundException" in str(e):
                        self._info(f"  Gateway already deleted")
                    elif "has targets" in str(e).lower():
                        self._info(f"  Gateway still has targets, retrying after wait...")
                        import time
                        time.sleep(10)
                        try:
                            bedrock_client.delete_gateway(gatewayIdentifier=gw_id)
                            self._success(f"  Gateway {gw_name} deleted (retry)")
                        except Exception as e2:
                            self._error(f"  Failed to delete gateway on retry: {e2}")
                    else:
                        self._error(f"  Failed to delete gateway: {e}")
            
            self._success(f"Cleaned up {len(workshop_gateways)} workshop gateway(s)")
            
        except Exception as e:
            self._error(f"Failed to clean up workshop gateways: {e}")
    ...
    def print_summary(self):
        ...
        if self.config and 'observability' in self.config:
            print(f"   - Observability Destination: {self.config['observability'].get('destination_id', 'N/A')}")
        if self.config and 'subagents' in self.config:
            for sa in self.config['subagents']:
                print(f"   - A2A Sub-Agent: {sa.get('name', 'N/A')} ({sa.get('agent_arn', 'N/A')})")
    ...
    def cleanup(self):
        ...
        try:
            ...
            # Delete gateway if exists (from config)
            self.delete_gateway()

            # Always clean up workshop gateways for strands (by prefix)
            if self.websocket_folder == 'strands-sonic' or getattr(self, 'cleanup_all_gateways', False):
                self.delete_all_workshop_gateways()

            # Delete A2A sub-agents
            self.delete_subagents()

            # Delete memory if exists
            self.delete_memory()

            # Delete observability destination if exists
            self.delete_observability()
...
Examples:
  ...
  python cleanup.py strands
    ...
    parser.add_argument(
        'websocket_folder',
        choices=['bedrock-sonic', 'strands-sonic', 'langchain-transcribe-polly', 'pipecat-sonic', 'echo'],
        help='Websocket folder to clean up (bedrock-sonic, strands-sonic, langchain-transcribe-polly, pipecat-sonic, or echo)'
    )
    
    parser.add_argument(
        '--all-gateways',
        action='store_true',
        help='Delete ALL gateways matching voice-agent-workshop-* or voice_workshop_* prefix (for cleaning up stale resources)'
    )
    ...
    cleanup.cleanup_all_gateways = args.all_gateways
    cleanup.cleanup()
