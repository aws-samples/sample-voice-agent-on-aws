...
if MCP_GATEWAY_ARNS and MCP_GATEWAY_URLS:
    try:
    ...
    logger.info("ℹ️  MCP Gateway not configured (set MCP_GATEWAY_ARNS and MCP_GATEWAY_URLS to enable tools)")

# Check if AgentCore Memory is configured
MEMORY_ID = os.getenv('MEMORY_ID')
MEMORY_REGION = os.getenv('AWS_DEFAULT_REGION', 'us-east-1')
voice_agent_memory = None

if MEMORY_ID:
    voice_agent_memory = VoiceAgentMemory(memory_id=MEMORY_ID, region=MEMORY_REGION)
    logger.info(f"✅ AgentCore Memory configured (ID: {MEMORY_ID})")
else:
    logger.info("ℹ️  AgentCore Memory not configured (set MEMORY_ID to enable)")
...
async def startup_event():
    ...
    if gateway_arns:
        ...
        logger.info("ℹ️  No MCP Gateways configured (agent will run without tools)")
    if voice_agent_memory:
        logger.info(f"🧠 Memory: enabled (ID: {MEMORY_ID})")
    else:
        logger.info("🧠 Memory: disabled (set MEMORY_ID env var to enable)")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    await handle_websocket_session(
        websocket,
        default_gateway_arns=gateway_arns,
        send_output=chunked_send_json,
        memory=voice_agent_memory,
    )
