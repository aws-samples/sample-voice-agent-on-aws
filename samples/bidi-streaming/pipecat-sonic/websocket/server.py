...
Follows the same server pattern as strands-sonic/langchain-transcribe-polly/bedrock-sonic agents
...
# ---------------------------------------------------------------------------
# IMDS credential helpers (same pattern as strands-sonic/bedrock-sonic/langchain servers)
# ---------------------------------------------------------------------------
...
async def refresh_credentials_periodically():
    ...
    while True:
        try:
            ...
            logger.warning(f"Credential refresh error: {e}")
...
SYSTEM_INSTRUCTION = (
    ...
    "You have access to a get_current_time tool — use it when the user asks about the time or date."
)
...
async def get_current_time(params: FunctionCallParams):
    ...
    await params.result_callback(
        {"time": now.strftime("%A, %B %d, %Y at %I:%M %p UTC")}
    )
...
tools = ToolsSchema(
    standard_tools=[
        FunctionSchema(
            ...
            description="Get the current system date and time in UTC",
            properties={},
            required=[],
        ),
    ]
...
async def start(request: Request):
    """Return the WebSocket URL for the Pipecat client.

    Detects CloudFront proxy from request headers and returns the appropriate
    wss:// URL. Falls back to ws://localhost for local development.
    ...
    # Check if request came through a proxy (CloudFront/nginx)
    forwarded_host = request.headers.get("x-forwarded-host") or request.headers.get("host", "")
    forwarded_proto = request.headers.get("x-forwarded-proto", "")

    if forwarded_proto == "https" or ".cloudfront.net" in forwarded_host:
        # Behind CloudFront proxy — construct wss:// URL via proxy path
        ws_url = f"wss://{forwarded_host}/proxy/{port}/ws"
    else:
        ws_url = f"ws://localhost:{port}/ws"

    logger.info(f"🔗 /start returning ws_url: {ws_url}")
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    try:
        ...
        llm.register_function("get_current_time", get_current_time, cancel_on_interruption=False)
        ...
        async def on_client_connected(transport, client):
            ...
            context.add_message(
                {"role": "user", "content": "Please greet the user warmly and ask how you can help."}
            )
