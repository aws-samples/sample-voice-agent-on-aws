...
async function connect() {
  ...
  try {
    ...
    // Use a relative path that works behind CloudFront proxy prefix
    const startUrl = serverUrl || new URL("start", window.location.href).href;
    ...
    // If behind a proxy (https), construct wss:// URL from current origin
    let wsUrl = data.ws_url;
    if (window.location.protocol === "https:") {
      const port = 8081;
      wsUrl = `wss://${window.location.host}/proxy/${port}/ws`;
    }
    log("WebSocket URL: " + wsUrl, "log-system");

    await pcClient.connect({ wsUrl: wsUrl });
